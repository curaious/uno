...
2. **Go SDK**: Install the Uno SDK with Restate support:
```bash
go get github.com/praveen001/uno
```
...
func main() {
    ...
    client.StartRestateService("0.0.0.0", "9080")
...
**Note**: If this process crashes, both components restart together. True durability requires separate processes (Option 2).
...
func main() {
    ...
    log.Println("Restate service starting on :9080")
    client.StartRestateService("0.0.0.0", "9080")
...
After starting your Restate service (on port 9080), you must register it with the Restate server so it can discover and invoke your agent workflows.
...
## Invoking a Durable Agent

Once your deployment is registered, you can invoke the agent through the application server:
...
func invokeDurableAgent() {
    input := map[string]any{
        "messages": []responses.InputMessageUnion{
            responses.UserMessage("Hello, what can you help me with?"),
        },
        "namespace": "default",
        "previous_message_id": "",
    }
    ...
    // Call the application server which invokes the Restate workflow
    resp, err := http.Post(
        "http://localhost:8070?agent=my-agent",
        "application/json",
    ...
    // The agent execution is now durable and will survive crashes
}
```

## Example: Complete Durable Agent

See the complete working example in the repository: [`examples/13_durable_agent/main.go`](https://github.com/praveen001/uno/tree/main/examples/13_durable_agent)
