...
func (t *DeleteUserTool) Tool(ctx context.Context) *responses.ToolUnion {
    return t.ToolUnion
}

func (t *DeleteUserTool) NeedApproval() bool {
...
```go
mcpClient, err := tools.NewMCPServer(ctx, "http://localhost:9001/sse", nil)
if err != nil {
    log.Fatal(err)
}
...
agent := client.NewAgent(&sdk.AgentOptions{
    ...
    Tools: mcpClient.GetTools(
        tools.WithMcpApprovalRequiredTools("delete_file", "send_email", "execute_command"),
    ),
})
```
...
result, err := agent.Execute(ctx, []responses.InputMessageUnion{approvalResponse}, callback)
```
