...
When multiple API keys are configured for a provider, Uno uses **weighted random selection** to choose which key to use for each request.

Each API key can be assigned a `Weight`. The probability of a key being selected is proportional to its weight relative to the sum of all weights for that provider.
