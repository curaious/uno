...
func createNativeResponseCreatedForGemini(id, model string) *responses.ResponseChunk {
...
func createNativeOutputTextDeltaForGemini(itemId string, outputIndex, contentIndex int, delta string, seqNum int) *responses.ResponseChunk {
	return &responses.ResponseChunk{
...
func createNativeOutputItemAddedFunctionCallForGemini(itemId string, outputIndex int, callId, name string, args any) *responses.ResponseChunk {
	argsStr := ""
	if args != nil {
		argsStr = args.(string)
	}
...
func TestNativeToGemini_NoOutputEvents(t *testing.T) {
	...
	inProgress := &responses.ResponseChunk{
		OfResponseInProgress: &responses.ChunkResponse[constants.ChunkTypeResponseInProgress]{
			...
			SequenceNumber: 1,
			Response:       responses.ChunkResponseData{Id: "resp_no_out"},
		},
	}
	...
	textDone := &responses.ResponseChunk{
	...
	contentPartAdded := &responses.ResponseChunk{
		OfContentPartAdded: &responses.ChunkContentPart[constants.ChunkTypeContentPartAdded]{
			...
			Part: responses.OutputContentUnion{
				OfOutputText: &responses.OutputTextContent{Text: ""},
			},
		},
	}
...
func TestNativeToGemini_OutputItemAddedMessage(t *testing.T) {
	...
	msgAdded := &responses.ResponseChunk{
		OfOutputItemAdded: &responses.ChunkOutputItem[constants.ChunkTypeOutputItemAdded]{
			Type:           constants.ChunkTypeOutputItemAdded("response.output_item.added"),
			SequenceNumber: 2,
			OutputIndex:    0,
...
func TestNativeToGemini_PartsOnlyWithContent(t *testing.T) {
	...
	fnDelta := &responses.ResponseChunk{
